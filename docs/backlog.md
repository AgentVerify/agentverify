...
The distribution verifier now checks both wheel contents and the console-script entry point, and
`--smoke-install` installs the wheel in a temporary virtualenv before running version, schema, and
safe-example scan commands through the installed `agentverify` executable.
