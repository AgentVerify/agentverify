...
Schema v129 preserves custom rejection-message provenance on source-proven OpenAI Agents run-state
reject decisions. Python `rejection_message=` and TypeScript `{ message }` reject options are
classified as literal, literal-binding, template, or dynamic without storing message text. The pinned
...
`agent.asTool({ runConfig: { workflowName } })` trace-workflow controls, and schema v142 adds exact
...
privacy-preserving transcription prompt/keyword metadata, exact top-level session model metadata,
and exact model-only session options. AgentVerify emits
...
settings. The Blaxel and Cloudflare sandbox extension examples show stable Runner workflow names
...
The curated regression set has reached 730 pinned positive/negative locations, with 2,435 separately
