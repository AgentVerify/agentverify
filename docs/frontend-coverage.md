...
framework, wrapper, or configuration path. Counts come from schema-v41
...
Across the selected snapshot, 8,744 agent/tool observations have module-qualified symbol IDs. Of
2,970 relationship endpoint observations, 1,936 carry IDs and 1,934 resolve to an observed component
...
edges resolve exactly. In Python files with repeated identities, schema v41 records 325 same-scope and
...
The native AI BOM 1.1 resolver independently classifies all 2,970 endpoints: 1,934 by symbol ID, 481
...
redirect disabling separately from unresolved DNS and redirect scope. The full schema-v41 corpus run
...
to unguarded edge-runtime fetch. Schema v41 therefore reports nine TypeScript secure-network
...
explicit loopback policy, and binds the endpoint to the network card source origin. Schema v41 thus
...
Schema v41 records 14 exact MCP-forwarding control edges: one explicit allowlist, three discovery
...
- Python network-origin controls require an exact module-level `urllib.parse` parser import, one
  ...
  preserved separately. Schema v41 observes zero such controls on the Python corpus reviews.
...
- Selected-path scans parse only selected files. The research corpus adds at most 20 local source
  ...
  import closure, all charged against the same cap. This refresh added 159 files across 17 repositories. Unselected definitions
  and controls are not evidence of repository-wide coverage or absence.

## Quality interpretation

The 330-label rule truth set and 246-label IR relationship set are curated regression suites. They
