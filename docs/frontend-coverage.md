...
framework, wrapper, or configuration path. Counts come from schema-v45
...
Across the selected snapshot, 8,748 agent/tool observations have module-qualified symbol IDs. Of
2,990 relationship endpoint observations, 1,941 carry IDs and 1,939 resolve to an observed component
(1,680 Python and 259 TypeScript); the two unmatched IDs are explicit Python re-export targets.
Repeated Python and TypeScript constructor bindings are occurrence-qualified. Their direct source
edges resolve exactly. In Python files with repeated identities, schema v45 records 325 same-scope and
...
The native AI BOM 1.2 resolver independently classifies all 2,990 endpoints: 1,939 by symbol ID, 492
by exact relationship evidence, 20 by a unique display name, 38 as ambiguous, and 501 as unresolved.
...
redirect disabling separately from unresolved DNS and redirect scope. The full schema-v45 corpus run
...
to unguarded edge-runtime fetch. Schema v45 therefore reports nine TypeScript secure-network
...
explicit loopback policy, and binds the endpoint to the network card source origin. Schema v45 thus
...
Schema v45 records 14 exact MCP-forwarding control edges: one explicit allowlist, three discovery
...
- Python network-origin controls require an exact module-level `urllib.parse` parser import, one
  ...
  preserved separately. Schema v45 observes zero such controls on the Python corpus reviews.
...
The 336-label rule truth set and 259-label IR relationship set are curated regression suites. They
