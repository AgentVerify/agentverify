...
framework, wrapper, or configuration path. Counts come from schema-v15
...
2,758 relationship endpoint observations, 1,875 carry IDs and 1,873 resolve to an observed component
(1,622 Python and 251 TypeScript); the two unmatched IDs are explicit Python re-export targets.
Repeated Python and TypeScript constructor bindings are occurrence-qualified. Their direct source
edges resolve exactly. In Python files with repeated identities, schema v15 records 325 same-scope and
...
The native AI BOM 1.1 resolver independently classifies all 2,758 endpoints: 1,873 by symbol ID, 338
...
Schema v15 records 14 exact MCP-forwarding control edges: one explicit allowlist, three discovery
...
The 185-label rule truth set and 62-label IR relationship set are curated regression suites. They
