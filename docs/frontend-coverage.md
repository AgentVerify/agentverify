...
`tool-choice-policy` controls on the source agent. The pinned `routing.ts` example contributes the
corpus
...
`required` and `programmatic_tool_calling` tool-choice policy.
