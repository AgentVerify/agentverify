...
framework, wrapper, or configuration path. Counts come from schema-v14
...
2,756 relationship endpoint observations, 1,875 carry IDs and 1,873 resolve to an observed component
...
edges resolve exactly. In Python files with repeated identities, schema v14 records 325 same-scope and
...
The native AI BOM 1.1 resolver independently classifies all 2,756 endpoints: 1,873 by symbol ID, 336
...
Schema v14 records 14 exact MCP-forwarding control edges: one explicit allowlist, three discovery
...
The 177-label rule truth set and 56-label IR relationship set are curated regression suites. They
