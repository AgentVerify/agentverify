...
framework, wrapper, or configuration path. Counts come from schema-v53
...
Across the selected snapshot, 8,927 agent/tool observations have module-qualified symbol IDs. Of
3,054 relationship endpoint observations, all 2,174 identified endpoints resolve to an observed
component (1,915 Python and 259 TypeScript). Two former false IDs on CrewAI test edges are now
...
edges resolve exactly. Schema v53 records 311 Python lexical-single-definition, 20 same-block
dominating-definition, and three same-class helper-return resolutions. One
`ambiguous-repeated-binding` reference remains withheld: a typed `ApplyPatchTool` parameter.
Literal Python Agent tool lists recover 150 exact callable tools—83 outside tests—with seven
capability edges and 162 exact Agent edges. Import-proven OpenAI `function_tool(function)`
...
The native AI BOM 1.2 resolver independently classifies all 3,054 endpoints: 2,174 by symbol ID, 521
by exact relationship evidence, 15 by a unique display name, 29 as ambiguous, and 315 as unresolved.
...
redirect disabling separately from unresolved DNS and redirect scope. The full schema-v53 corpus run
...
to unguarded edge-runtime fetch. Schema v53 therefore reports nine TypeScript secure-network
...
the corpus to 18 AV-NET001 reviews across 11 repositories. A guarded transport, fixed argument map,
...
explicit loopback policy, and binds the endpoint to the network card source origin. Schema v53 thus
...
Schema v53 records 14 exact MCP-forwarding control edges: one explicit allowlist, three discovery
...
edge and a path derived from a tool input. Expanded registry and direct-callable reachability produces
32 filesystem reviews across ten repositories. Four CrewAI Examples sinks are specialized as
AV-FS002, leaving 28 AV-FS001 sites across nine repositories. Fixed paths remain inventory-only, and attribute `.open()`
...
- Python network-origin controls require an exact module-level `urllib.parse` parser import, one
  ...
  preserved separately. Schema v53 observes zero such controls on the Python corpus reviews.
...
The 342-label rule truth set and 322-label IR relationship set are curated regression suites. They
