...
framework, wrapper, or configuration path. Counts come from schema-v44
...
Across the selected snapshot, 8,747 agent/tool observations have module-qualified symbol IDs. Of
2,980 relationship endpoint observations, 1,939 carry IDs and 1,937 resolve to an observed component
(1,678 Python and 259 TypeScript); the two unmatched IDs are explicit Python re-export targets.
Repeated Python and TypeScript constructor bindings are occurrence-qualified. Their direct source
edges resolve exactly. In Python files with repeated identities, schema v44 records 325 same-scope and
...
redirect disabling separately from unresolved DNS and redirect scope. The full schema-v44 corpus run
...
to unguarded edge-runtime fetch. Schema v44 therefore reports nine TypeScript secure-network
...
explicit loopback policy, and binds the endpoint to the network card source origin. Schema v44 thus
...
Schema v44 records 14 exact MCP-forwarding control edges: one explicit allowlist, three discovery
...
- Python network-origin controls require an exact module-level `urllib.parse` parser import, one
  ...
  preserved separately. Schema v44 observes zero such controls on the Python corpus reviews.
...
The 336-label rule truth set and 255-label IR relationship set are curated regression suites. They
