...
framework, wrapper, or configuration path. Counts come from schema-v30
...
edges resolve exactly. In Python files with repeated identities, schema v30 records 325 same-scope and
...
redirect disabling separately from unresolved DNS and redirect scope. The full schema-v30 corpus run
...
Schema v30 records 14 exact MCP-forwarding control edges: one explicit allowlist, three discovery
...
- Python network-origin controls require an exact module-level `urllib.parse` parser import, one
  ...
  preserved separately. Schema v30 observes zero such controls on the 13 corpus reviews.
...
The 298-label rule truth set and 189-label IR relationship set are curated regression suites. They
