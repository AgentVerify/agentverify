...
framework, wrapper, or configuration path. Counts come from schema-v87
...
- Browser-page evaluation inventories 80 import-context calls and proves 35 receivers: two exact
  ...
  calls through an exact constructor-bound page field or immutable local alias. Class or
  ...
  conditional, repeated, late, or shadowed-factory fields are withheld. The other 45 fixed-script
  observations retain unresolved receiver state. Dynamic promotion also supports one
  immutable alias of a typed parameter; untyped/inherited fields, chained locators, sanitizers,
  structured JavaScript builders, imported/transitive helpers, and evaluator APIs beyond
  `.evaluate(...)` remain unresolved.
...
The 533-label rule truth set and 857-label IR component/relationship set are curated regression
