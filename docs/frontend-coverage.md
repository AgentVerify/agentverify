...
framework, wrapper, or configuration path. Counts come from schema-v159
...
Literal `entries` Manifest sources are recorded as `sandbox-manifest-entry` controls for exact
`file()`, `gitRepo()`, direct `{ type: 'file' }`, and literal/immutable `localDir()` forms; file
contents are deliberately not copied into IR.
Literal `root` Manifest workspace paths are recorded as `sandbox-workspace-root` controls for direct
or earlier immutable module-level string bindings; dynamic roots and unknown Manifest constructors
remain unresolved.

Literal local sandbox `snapshot` options are recorded as `sandbox-state-persistence` controls when
an exact `@openai/agents/sandbox/local` client provides `type: "local"` and a direct or earlier
immutable module-level `baseDir`; unknown constructors and dynamic snapshot directories remain
unresolved.
Literal `sandbox.concurrencyLimits` runtime options are recorded as
`sandbox-concurrency-limit` controls when every limit value is a direct integer literal and the same
option object has an exact sandbox runtime client/session.
...
or imported settings objects remain unresolved.
For hosted web search, exact
`webSearchTool({ filters: { allowedDomains }, searchContextSize, userLocation })` objects emit
`web-search-policy` controls on the tool and copy the same scope metadata onto the
...
agent requests `web_search_call.action.sources`. Dynamic domain/context/location/include bindings
remain unresolved.
OpenAI Realtime `RealtimeAgent` constructors are source-agent inventory, and exact
`RealtimeSession(agent, { model })` or
`RealtimeSession(agent, { model, config: { parallelToolCalls, outputModalities, audio, reasoning } })`
literal settings emit `realtime-session-config-policy` controls when the session's first argument
resolves to a known RealtimeAgent. Direct static top-level session model strings, including
model-only options, literal audio input/output formats, and literal
`audio.input.transcription.{model, delay, prompt, keywords, languages}` values are recorded as
...
exactly one tool uses that helper, including exact named reexports and unambiguous star reexports.
Shared imported helpers and ambiguous reexported helper modules remain unresolved until the
caller-tool provenance is unambiguous.
...
agent with the literal turn cap. Exact delegated `agent.asTool({ runConfig: { model } })` calls emit
`agent-model-override` controls on the delegated agent with provider, model, and shallow literal
settings metadata. Exact delegated `agent.asTool({ runConfig: { workflowName } })` calls emit
`trace-workflow` controls on the delegated agent with literal workflow-name provenance. Stable exact
`new Runner({ workflowName })` instances emit per-run `trace-workflow` controls when later
...
tracing-disablement controls plus eight-turn delegated limits and delegated workflow names, and the
translator agents-as-tools example contributes a three-turn delegated limit plus a delegated
`gpt-5.4` model override with low reasoning/verbosity settings. The tool-search example contributes
...
The 733-label rule truth set and 2,583-label IR component/relationship set are curated regression
