...
`audio.input.transcription.{model, delay, languages}` values are recorded as session governance
metadata; dynamic realtime session config values remain unresolved.
