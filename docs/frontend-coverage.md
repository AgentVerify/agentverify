...
framework, wrapper, or configuration path. Counts come from schema-v8
...
2,722 relationship endpoint observations, 1,871 carry IDs and 1,869 resolve to an observed component
(1,622 Python and 247 TypeScript); the two unmatched IDs are explicit Python re-export targets.
Repeated Python and TypeScript constructor bindings are occurrence-qualified. Their direct source
edges resolve exactly. In Python files with repeated identities, schema v8 records 325 same-scope and
...
The native AI BOM 1.1 resolver independently classifies all 2,722 endpoints: 1,869 by symbol ID, 306
...
`registerTool` observations, 70 generic/Mastra object-property tools, and six exact
callback-to-capability edges; the factory/property categories overlap. Literal or uniquely bound
...
- TypeScript network flow is callback-local. Calls through local/imported helpers, Axios request
  objects, computed callback properties, redirects, and hostname-validation helpers remain unresolved.
...
The 166-label rule truth set and 30-label IR relationship set are curated regression suites. They
