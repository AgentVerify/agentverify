...
framework, wrapper, or configuration path. Counts come from schema-v86
...
- Browser-page evaluation inventories 80 import-context calls and proves 22 receivers: two exact
  Playwright-annotated SWE-agent parameters, five Skyvern calls reached from an exact imported page
  factory, and 15 exact class-attribute annotations across CAMEL, LaVague, and MetaGPT. Class or
  `__init__` annotations may use an exact immutable Playwright import under `TYPE_CHECKING`.
  Conflicting/wrong annotations, near or rebound imports, and static methods are withheld. The other
  58 fixed-script observations retain unresolved receiver state. Dynamic promotion also supports one
...
The 525-label rule truth set and 847-label IR component/relationship set are curated regression
