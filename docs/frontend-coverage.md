...
`RealtimeSession(agent, { config: { parallelToolCalls, outputModalities, audio, reasoning } })`
literal settings emit `realtime-session-config-policy` controls when the session's first argument
resolves to a known RealtimeAgent. Direct literal audio input/output formats and literal
...
config values remain unresolved.
