...
framework, wrapper, or configuration path. Counts come from schema-v64
...
redirect disabling separately from unresolved DNS and redirect scope. The full schema-v64 corpus run
...
- MCP package launchers require literal `npx`/`uvx` commands in MCP JSON, a literal nested Python
  `mcpServers` dictionary, or a known constructor imported from an MCP module. Dynamic package
  selection, shell wrappers, `npx --package`/`--call`, module-qualified or rebound constructors,
...
The 406-label rule truth set and 512-label IR component/relationship set are curated regression suites. They
