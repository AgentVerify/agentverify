...
framework, wrapper, or configuration path. Counts come from schema-v135
...
`RealtimeSession(agent, { config: { parallelToolCalls: true|false } })` settings emit
`realtime-session-config-policy` controls when the session's first argument resolves to a known
RealtimeAgent. Dynamic realtime session config values remain unresolved.
...
Cloudflare sandbox extension examples contribute Runner workflow names for normal and streaming branches; the sandbox
capabilities and hosted MCP human-in-the-loop examples contribute direct-run and Runner-run
`maxTurns` bounds; the forcing-tool-use and programmatic tool-calling examples contribute
`required` and `programmatic_tool_calling` Agent-level tool-choice policy; the hosted MCP
human-in-the-loop example also contributes Runner-level `required` initial-run and `auto`
resume-run tool-choice policy; the voice-agent configure-session example contributes realtime
session-level parallel tool-call configuration.
...
The 729-label rule truth set and 2,086-label IR component/relationship set are curated regression
