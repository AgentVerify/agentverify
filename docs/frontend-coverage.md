...
`RealtimeSession(agent, { config: { parallelToolCalls, reasoning: { effort } } })` literal settings
emit `realtime-session-config-policy` controls when the session's first argument resolves to a known
