...
`trace-workflow` controls on the delegated agent with literal workflow-name provenance. The pinned
`routing.ts` example contributes the corpus
...
`gpt-5.4` model override with low reasoning/verbosity settings.
