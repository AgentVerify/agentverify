...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 16 default-scope matches across 9 repositories; schema v37 finds zero exact Python allowlists on the original Python reviews, one configured-open TypeScript policy, two fully pinned CrewAI, two proxy-conditional Composio, ten configurable Langflow paths, one default-off n8n composition with unresolved proxy behavior, one default-on Flowise Axios path with environment-dependent proxy pinning, and one default-on Flowise `node-fetch` path with a post-spread pinned agent |
