...
| AV-EXEC001 | finding | high | Dynamic command executed through a system shell | 24 default-scope engine matches after schema v119 resolves Letta Code's default Bash tool through an explicit `zsh`/`bash -c` launcher, schema v118 resolves Roo Code's native model command into `terminal.runCommand`, and schema v117 resolves Trae Agent's persistent Bash stdin path |
