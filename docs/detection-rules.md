...
| AV-AG001 | inventory | info | Identify agent framework, agent definitions, handoffs, and graph edges | Exact framework signatures cover 23 families |
...
| AV-FS001 | review | high | Agent tool writes, copies, moves, or deletes a tool-input-controlled filesystem path without a proven narrow path-boundary control | 37 default-scope matches across 9 repositories; schema v116 adds six reachable OpenHands `FileEditorTool` instances whose workspace is not a containment boundary, while schema v113 removes Qwen-Agent's hashed-cache false positive through an exact imported hexadecimal SHA-256 path-segment sanitizer |
