...
| AV-FS001 | review | high | Agent tool writes, copies, moves, or deletes a tool-input-controlled filesystem path without a proven narrow path-boundary control | 31 default-scope matches across 8 repositories; schema v113 removes Qwen-Agent's hashed-cache false positive by proving an exact imported hexadecimal SHA-256 path-segment sanitizer, while lookalikes, rebound helpers, digest-derived roots, and extra unsanitized segments remain reviews |
