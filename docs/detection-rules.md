...
| AV-EXEC002 | finding | high | Dynamic input reaches an evaluator/interpreter | 51 default-scope matches across 15 repositories; browser-page promotion requires an exact parameter/alias, typed class attribute, or pinned factory receiver, while exact hosted-sandbox `CodeInterpreterTool` assets remain inventory |
