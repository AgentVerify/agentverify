...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 14 default-scope matches across 8 repositories; schema v35 finds zero exact Python allowlists on the original Python reviews, one configured-open TypeScript policy, two fully pinned CrewAI, two proxy-conditional Composio, ten configurable Langflow paths, and one default-off n8n composition with unresolved proxy behavior |
