...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 18 default-scope matches across 11 repositories; schema v42 adds Composio CLI's schema-gated tool-argument URL upload to the existing Python/TypeScript reviews and separately preserves the exact Python, n8n, Flowise, Google ADK, Activepieces, and Composio conditional-runtime control states |
