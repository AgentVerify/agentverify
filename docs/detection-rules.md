...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 15 default-scope matches across 9 repositories; schema v36 finds zero exact Python allowlists on the original Python reviews, one configured-open TypeScript policy, two fully pinned CrewAI, two proxy-conditional Composio, ten configurable Langflow paths, one default-off n8n composition with unresolved proxy behavior, and one default-on Flowise request-object path with environment-dependent proxy pinning |
