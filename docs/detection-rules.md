...
| AV-EXEC002 | finding | high | Dynamic input reaches an evaluator/interpreter | 51 default-scope matches across 15 repositories; browser-page promotion supports API-specific script arguments for `evaluate`, `evaluate_handle`, `eval_on_selector`, `eval_on_selector_all`, and `evaluate_all` and requires a provenance-backed parameter, constructor/lifecycle field or property, local/module page, exact local context-manager yield, unanimous private same-class or same-module helper parameter, derived Locator, or pinned factory receiver, while exact hosted-sandbox `CodeInterpreterTool` assets remain inventory |
...
| AV-APPROVAL002 | review | high | Reachable local OpenAI Agents Python/TypeScript shell tool has approval disabled or exposes no SDK approval hook | 2 default-scope matches plus 5 test-scoped exact Python `LocalShellTool` assets with no approval parameter; the executor may still enforce an equivalent control, and the separate Python MCP composition remains inventory because it lacks a proven destructive capability |
...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 23 default-scope matches across 12 repositories; schema v65 retains five exact Google ADK Python project-local helper paths, Composio CLI's schema-gated URL upload, and proof that Google ADK OpenAPI model arguments cannot replace the spec-configured origin alongside the other exact Python and TypeScript control states; schema v85 keeps exact provider-hosted OpenAI web search as inventory because no caller-selected origin is proven |
