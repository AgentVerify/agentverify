...
| AV-EXEC002 | finding | high | Dynamic input reaches an evaluator/interpreter | 51 default-scope matches across 15 repositories; browser-page promotion supports API-specific script arguments for `evaluate`, `evaluate_handle`, `eval_on_selector`, `eval_on_selector_all`, and `evaluate_all` and requires a provenance-backed parameter, field/property, local/module page, derived Locator, or pinned factory receiver, while exact hosted-sandbox `CodeInterpreterTool` assets remain inventory |
