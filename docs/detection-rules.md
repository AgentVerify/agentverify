...
| AV-APPROVAL010 | review | high | Spawned sub-agent loses the parent tool policy and approval callback | 1 exact Cline VS Code production path; `spawn_agent` is default-enabled and absent from the root explicit approval list, then the session wrapper creates child shell/edit tools without forwarding either supported approval input |
