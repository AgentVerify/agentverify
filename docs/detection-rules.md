...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 17 default-scope matches across 10 repositories; schema v40 finds zero exact Python allowlists on the original Python reviews, one configured-open TypeScript policy, two fully pinned CrewAI, two proxy-conditional Composio, ten configurable Langflow paths, one default-off n8n composition, two default-on Flowise paths with distinct proxy behavior, one always-on Google ADK preflight with unpinned global fetch, and one always-on Activepieces filtering-Axios path for a configured MCP origin |
