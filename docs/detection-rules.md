...
| AV-APPROVAL002 | review | high | Reachable local OpenAI Agents Python/TypeScript shell tool uses the SDK's disabled approval policy | 2 default-scope matches in the pinned SDK examples; schema v54 separately inventories one disabled-default agent→MCP binding without promoting it absent a proven destructive MCP capability |
| AV-FS001 | review | high | Agent tool writes, copies, moves, or deletes a tool-input-controlled filesystem path without a proven narrow path-boundary control | 28 default-scope matches across 9 repositories after exact Python callable-tool recovery and fixed-path filtering |
...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 18 default-scope matches across 11 repositories; schema v54 retains Composio CLI's schema-gated URL upload while proving Google ADK OpenAPI model arguments cannot replace the spec-configured origin, alongside the exact Python, n8n, Flowise, Google ADK load-page, Activepieces, and Composio conditional-runtime control states |
