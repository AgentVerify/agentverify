...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 18 default-scope matches across 11 repositories; schema v46 retains Composio CLI's schema-gated URL upload while proving Google ADK OpenAPI model arguments cannot replace the spec-configured origin, alongside the exact Python, n8n, Flowise, Google ADK load-page, Activepieces, and Composio conditional-runtime control states |
...
| AV-AUDIT001 | warning | medium | Consequential action has no attributable durable audit edge | OpenTelemetry spans prove instrumentation only; Google ADK proves attributable durable test storage, while one production Skyvern SQL action record has unresolved actor identity and best-effort delivery; general absence coverage is not ready |
