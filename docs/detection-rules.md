...
| AV-AG001 | inventory | info | Identify agent framework, agent definitions, handoffs, and graph edges | Exact framework signatures cover 24 families |
...
| AV-EXEC001 | finding | high | Dynamic command executed through a system shell | 22 default-scope engine matches after schema v117 resolves Trae Agent's default model-controlled persistent Bash stdin path |
...
| AV-APPROVAL002 | review | high | Reachable local agent shell tool has approval disabled or exposes no per-action decision hook | 3 default-scope matches: two OpenAI Agents shell paths plus Trae Agent's exact default host-local Bash tool; 5 test-scoped Python `LocalShellTool` assets remain inventory, and unresolved executors are not treated as missing controls |
...
| AV-FS001 | review | high | Agent tool writes, copies, moves, or deletes a tool-input-controlled filesystem path without a proven narrow path-boundary control | 38 default-scope matches across 10 repositories; schema v117 adds Trae Agent's absolute-only but root-unconstrained default editor, schema v116 adds six OpenHands file editors, and schema v113 removes Qwen-Agent's hashed-cache false positive through exact SHA-256 segment proof |
