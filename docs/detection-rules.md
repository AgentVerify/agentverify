...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 18 default-scope matches across 11 repositories; schema v45 retains Composio CLI's schema-gated URL upload while proving Google ADK OpenAPI model arguments cannot replace the spec-configured origin, alongside the exact Python, n8n, Flowise, Google ADK load-page, Activepieces, and Composio conditional-runtime control states |
...
| AV-AUDIT001 | warning | medium | Consequential action has no attributable durable audit edge | OpenTelemetry spans prove instrumentation only; one exact Google ADK BigQuery path proves durable attributable storage, but no production deployment or general absence coverage yet |
