...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 18 default-scope matches across 11 repositories; schema v44 retains Composio CLI's schema-gated URL upload while proving Google ADK OpenAPI model arguments cannot replace the spec-configured origin, alongside the exact Python, n8n, Flowise, Google ADK load-page, Activepieces, and Composio conditional-runtime control states |
