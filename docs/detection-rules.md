...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 23 default-scope matches across 12 repositories; schema v55 adds five exact Google ADK Python project-local helper paths, retains Composio CLI's schema-gated URL upload, and proves Google ADK OpenAPI model arguments cannot replace the spec-configured origin alongside the other exact Python and TypeScript control states |
