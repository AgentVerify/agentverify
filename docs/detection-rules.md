...
| AV-EXEC001 | finding | high | Dynamic command executed through a system shell | 23 default-scope engine matches after schema v118 resolves Roo Code's native model command into `terminal.runCommand` and schema v117 resolves Trae Agent's persistent Bash stdin path |
