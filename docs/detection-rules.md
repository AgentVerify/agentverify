...
| AV-APPROVAL011 | review | high | Computer-use safety-check callback acknowledges every pending safety check | OpenAI Agents JS example, OpenAI Agents Python SDK test IR evidence, and local JS/Python fixtures; no-handler, explicit empty-acknowledgement, and Python `return False` callbacks remain negative |
