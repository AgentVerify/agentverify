...
| AV-NET001 | review | high | Agent tool sends an HTTP request to a parameter-controlled origin | 13 default-scope matches across 7 repositories; schema v33 finds zero exact Python allowlists on those reviews and one configured-open TypeScript policy, while separately inventorying two fully pinned CrewAI and two proxy-conditional Composio paths |
