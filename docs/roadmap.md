...
| 4 | Approval coverage and bypass paths | 5 | 4 | 3 | 60 | 52 approval-positive; 18 auto-approval candidates; three exact environment-backed callbacks reach privileged OpenAI built-ins; three OpenAI Agents JS and two Agno reviews connect writable local filesystem MCP tools to disabled approval/confirmation defaults; schema v116 adds one OpenHands analyzer-only conversation whose SDK default remains `NeverConfirm`, with the paired `ConfirmRisky` example as a guarded control; schema v83 also inventories five test-scoped Python LocalShellTools whose SDK exposes no approval parameter, while their executors remain possible compensating controls |
| 5 | Filesystem scope and destructive writes | 5 | 4 | 3 | 60 | 59 writable-filesystem repos; 37 AV-FS001 reviews span 9 repositories after schema v116 resolves six reachable OpenHands file editors whose workspace is not containment, while schema v113 proves Qwen-Agent's tool-controlled URL can influence only a hexadecimal SHA-256 cache segment beneath its configured root |
...
- Support OpenAI, Anthropic, Azure OpenAI, Google, AWS Bedrock, Mistral, Groq, Cohere, Ollama,
  ...
  MetaGPT, Marvin, AgentScope, OpenHands SDK, Vercel AI SDK, and common MCP configuration shapes. Newer provider and
