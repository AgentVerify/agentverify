...
| 4 | Approval coverage and bypass paths | 5 | 4 | 3 | 60 | 52 approval-positive; 18 auto-approval candidates; schema v118 adds Roo Code's exact command auto-approval path and its raw-prefix allowlist seam; three exact environment-backed callbacks reach privileged OpenAI built-ins; schema v117 adds Trae Agent's default host-local Bash path whose direct executor has no per-action decision hook; schema v116 adds one OpenHands analyzer-only conversation at `NeverConfirm`; three OpenAI Agents JS and two Agno reviews connect writable MCP tools to disabled approval/confirmation defaults |
| 5 | Filesystem scope and destructive writes | 5 | 4 | 3 | 60 | 59 writable-filesystem repos; 38 AV-FS001 reviews span 10 repositories after schema v117 resolves Trae Agent's default editor, whose absolute-path check does not constrain a root; schema v116 resolves six OpenHands file editors, while schema v113 proves Qwen-Agent's URL controls only a SHA-256 cache segment |
