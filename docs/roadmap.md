...
|---:|---|---:|---:|---:|---:|---|
| 1 | Agent bill of materials: models, providers, frameworks, tools, MCP | 4 | 5 | 5 | 100 | 44 multi-provider research signals; exact selected-path framework/provider evidence now covers 48/49 repos, including schema v120's Continue CLI plan-mode runtime, schema v119's Letta Code identity and default Bash/Write registrations, schema v118's Roo Code identity and native command/approval path, schema v117's Trae Agent identity and exact default Bash/editor registrations, schema v116's two OpenHands repositories and 11 built-in-tool registrations, schema v115's current AutoGen package family and 17 exact OpenAI/Azure/Anthropic wrapper calls, schema v114's Dify Agent run composition, symbol-specific AgentScope, dedicated PydanticAI provider/model/embedding attribution, 220 exact Agno model-wrapper calls, 32 official TypeScript AI SDK calls, 23 native OpenAI/Anthropic/Google TypeScript SDK constructors across exact ESM plus immutable direct CommonJS imports, and 15 native request calls including one unanimous typed-parameter helper chain, five exact readonly class fields, and two private lazy accessors; 29 literal TypeScript models include three independently proven through immutable module constants across native and AI SDK calls, while generic or custom compatible endpoints remain withheld; it inventories exact-import OpenAI SandboxAgents, local shell, hosted code-interpreter, web-search, file-search, and image-generation tools, OpenHands, Trae, Roo, Letta, and Continue privileged built-ins, conditional Dify shell layers, and project-local MCP adapters, resolves direct local Agent factories, and links Python Agent→assigned/context-managed/imported-adapter MCP→registered-tool paths; 50 have MCP signals and 69 have privileged-capability signals |
| 2 | Dangerous shell and code execution | 5 | 4 | 4 | 80 | 42 shell-capable repos; 25 AV-EXEC001 findings after schema v120 resolves Continue CLI's selected plan-mode Bash tool, schema v119 resolves Letta Code's default Bash tool, schema v118 resolves Roo Code's native `execute_command` into local `terminal.runCommand`, and schema v117 resolves Trae Agent's persistent `/bin/bash` stdin; schema v84 adds five exact hosted-sandbox CodeInterpreterTool assets without misclassifying them as unsandboxed evaluation |
| 3 | MCP server/tool trust-boundary analysis | 5 | 4 | 4 | 80 | 50 MCP repos; schema v121 resolves Continue plan mode's wildcard approval of server-discovered tools without a local effect classification; 49 forwarding reviews, 34 automatic unpinned/floating package installs, one exact Semantic Kernel sampling auto-approval path, three generic sampling handlers without a proven user decision, three automatic elicitation acceptances, and two human-confirmed URL elicitations without full-URL display |
| 4 | Approval coverage and bypass paths | 5 | 4 | 3 | 60 | 53 approval-positive; schema v123 proves two Cline `spawn_agent` delegation paths: VS Code default-enables an unlisted parent spawn tool, while CLI sandbox mode can approve parent delegation; both drop the root policy/callback before child shell/edit tools; schema v121 isolates Continue CLI plan mode's unclassified MCP wildcard while schema v120 isolates its high-risk/unknown shell escalation collapse, preserving normal-mode prompts, critical shell blocking, and non-default mode; schema v119 resolves Letta Code's unrestricted default while preserving explicit-deny and always-ask precedence; schema v118 adds Roo Code's exact command auto-approval path and its raw-prefix allowlist seam; three exact environment-backed callbacks reach privileged OpenAI built-ins; schema v117 adds Trae Agent's default host-local Bash path whose direct executor has no per-action decision hook; schema v116 adds one OpenHands analyzer-only conversation at `NeverConfirm`; three OpenAI Agents JS and two Agno reviews connect writable MCP tools to disabled approval/confirmation defaults |
| 5 | Filesystem scope and destructive writes | 5 | 4 | 3 | 60 | 59 writable-filesystem repos; 39 AV-FS001 reviews span 11 repositories after schema v119 resolves Letta Code's default Write tool and schema v117 resolves Trae Agent's default editor; Letta's cross-agent guard and optional workspace sandbox remain explicit but do not prove a general default workspace-root boundary, while schema v113 proves Qwen-Agent's URL controls only a SHA-256 cache segment |
...
| 7 | Parameter-controlled network origins | 5 | 2 | 4 | 40 | 18 reviews across 11 repositories; schema v112 removes five Google ADK Python false positives by tracing helper arguments to immutable imported GitHub origins, while Composio CLI retains a schema-gated tool-argument URL upload through raw fetch and Google ADK OpenAPI provides a fixed-origin counterexample with encoded model path segments, alongside the existing exact Python and TypeScript control states |
...
- Support OpenAI, Anthropic, Azure OpenAI, Google, AWS Bedrock, Mistral, Groq, Cohere, Ollama,
  ...
  MetaGPT, Marvin, AgentScope, OpenHands SDK, Roo Code, Trae Agent, Vercel AI SDK, and common MCP configuration shapes. Newer provider and
