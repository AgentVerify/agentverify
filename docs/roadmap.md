...
|---:|---|---:|---:|---:|---:|---|
| 1 | Agent bill of materials: models, providers, frameworks, tools, MCP | 4 | 5 | 5 | 100 | 44 multi-provider research signals; schema v107 proves exact selected-path framework/provider evidence in 40/49 repos, including symbol-specific AgentScope, dedicated PydanticAI provider/model/embedding attribution, 220 exact Agno model-wrapper calls, 32 official TypeScript AI SDK calls, 23 native OpenAI/Anthropic/Google TypeScript SDK constructors across exact ESM plus immutable direct CommonJS imports, and four native model calls including one unanimous typed-parameter helper chain while withholding generic or custom compatible endpoints; it inventories exact-import OpenAI SandboxAgents, local shell, hosted code-interpreter, web-search, file-search, and image-generation tools plus project-local MCP adapters, resolves direct local Agent factories, and links Python Agent→assigned/context-managed/imported-adapter MCP→registered-tool paths; 50 have MCP signals and 69 have privileged-capability signals |
