...
|---:|---|---:|---:|---:|---:|---|
| 1 | Agent bill of materials: models, providers, frameworks, tools, MCP | 4 | 5 | 5 | 100 | 44 multi-provider research signals; schema v84 proves exact selected-path framework/provider evidence in 40/49 repos, inventories exact-import OpenAI SandboxAgents, LocalShellTools, CodeInterpreterTools, and project-local MCP adapters, resolves direct local Agent factories, and links Python Agent→assigned/context-managed/imported-adapter MCP→registered-tool paths; 50 have MCP signals and 69 have privileged-capability signals |
