...
|---:|---|---:|---:|---:|---:|---|
| 1 | Agent bill of materials: models, providers, frameworks, tools, MCP | 4 | 5 | 5 | 100 | 44 multi-provider research signals; schema v79 proves exact selected-path framework/provider evidence in 40/49 repos, distinguishes symbol-specific provider wrappers from import-only presence, and links Python Agent→stdio/in-process MCP→registered-tool paths; 50 have MCP signals and 69 have privileged-capability signals |
...
| 4 | Approval coverage and bypass paths | 5 | 4 | 3 | 60 | 52 approval-positive; 18 auto-approval candidates; three exact environment-backed callbacks reach privileged OpenAI built-ins; three OpenAI Agents JS and two Agno reviews connect writable local filesystem MCP tools to disabled approval/confirmation defaults, while four production Python Agent→MCP bindings retain unresolved generic approval coverage |
