...
| 3 | MCP server/tool trust-boundary analysis | 5 | 4 | 4 | 80 | 50 MCP repos; 49 forwarding reviews plus 34 automatic unpinned/floating package installs across five pinned repositories |
| 4 | Approval coverage and bypass paths | 5 | 4 | 3 | 60 | 52 approval-positive; 18 auto-approval candidates; three exact environment-backed callbacks reach privileged OpenAI built-ins; three OpenAI Agents JS reviews connect writable local filesystem MCP tools to a disabled approval default, while a Python agent→MCP binding still lacks a destructive-capability claim |
