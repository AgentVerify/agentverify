...
organizational trust root. The CI workflow runs the same helper and fails closed if detached
signature verification stops working. The helper never writes private-key material; it writes only
the exported signing payload, detached signature bundle, and public-key trust root in a temporary
directory.

```console
python scripts/verify_signed_policy_example.py
```
