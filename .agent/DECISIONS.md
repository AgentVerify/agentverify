...
- Decision: Treat `new WorkflowAgent(...)` as an agent only when `WorkflowAgent` is an unshadowed
  ...
  construction. A direct relative named import may also resolve when it points to one immutable
  exported const tool-set object whose entries are concrete object tools; the edge target remains
  the sibling module's tool component ID and records `tool_set_resolution:
  imported-local-tools-object`. Inline tools objects may link only to already-inventoried direct
  entries.
- Evidence: Vercel AI's pinned `examples/next-workflow/workflow/agent-chat.ts` imports
  ...
  toolsFromModule } from "./tools"` resolving to `ts:tools.ts#tool:*`, including a delegated
  helper-mapped code-execution tool, but the pinned public corpus does not yet contain this
  cross-file Vercel shape.
...
- Revisit when: Reexported tool-set modules, imported/reexported WorkflowAgent constructors, model
  helper modules, or approval-resumption flows can be resolved with comparable provenance and
  mutation guards.
