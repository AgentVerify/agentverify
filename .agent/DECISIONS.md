...
- Evidence: The SDK implementation and tests show `require_approval` can be disabled, always
  ...
  semantics. Same-block literal bindings and exact imported or named-reexported local literal
  approval policies are resolved while mutated imports/reexports remain dynamic. Remote URLs are
...
- Decision: Record exact Python `HostedMCPTool(tool_config={...})` approval state through
  ...
  `{always, never}` tool-list dict policies, imported and named-reexported local literal whole
  `tool_config` dictionaries, dynamic/imported/reexported-mutated binding metadata, and configured
  `on_approval_request` callbacks.
...
- Revisit when package-level or star-reexported policy or `tool_config` dictionaries, `Mcp(...)`
  factory objects in production examples, or callable `require_approval` policies can be resolved
  without broad Python dataflow.
