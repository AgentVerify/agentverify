...
- Evidence: The local hosted MCP fixture includes `onApproval: async () => ({ approve: false })`,
  ...
  callback parameters, nested numeric/boolean request fields, exact negated literal-set membership,
  and exact negated string-method predicates as inventory, not full human-review proof.
...
- Decision: Represent exact TypeScript `codexTool(...)` calls from
  ...
  `@openai/agents` `Agent` includes the tool through a stable same-file binding, exact relative
  imported/reexported binding, or an inline call in a literal `tools` array. Record
  `defaultThreadOptions` governance metadata, including
...
- Evidence: The local `typescript_openai_codex_tool` fixture covers same-file binding, inline
  generic-Agent usage, direct imported binding, exact named reexport, unambiguous star reexport,
  ambiguous two-source barrel negative, and mutated-binding negative. The pinned OpenAI Agents JS
...
- Revisit when richer working-directory provenance or reportable approval-policy findings can be
  tied to exact agent/tool identity without broad helper matching.
...
- Decision: Record exact `hostedMcpTool({...})` approval state through `mcp_approval_*` attributes
  ...
  comparisons, prefix/contains, literal-set, negated literal-set, and negated prefix/contains
  predicate metadata.
- Evidence: The pinned OpenAI Agents JS `hostedMcpTool` implementation maps omitted or `"never"`
  ...
  literal comparisons, request-field `startsWith`, request-field `includes`, array-literal
  membership predicates, negated array-literal membership predicates, and negated string-method
  predicates, plus local-array/local-binding lookalike negatives.
