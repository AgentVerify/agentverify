...
- OpenAI Agents JS hosted web-search scope is now reviewable when it is literal. Exact
  ...
  options extensions are covered in the current 2,609-label IR corpus.
...
- OpenAI Agents JS `hostedMcpTool({...})` approval settings are now visible as hosted-MCP-specific
  ...
  expressions such as `item.name !== "delete_page"` now record shallow predicate metadata, while
  local-variable lookalikes and opaque callback helpers are inventoried without request-review
  proof. The local fixture plus pinned OpenAI hosted MCP examples cover 24/24 hosted-MCP approval
  labels in the current 2,609-label IR corpus.
