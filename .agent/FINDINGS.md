...
- Policy digest trust roots are now generated by the CLI from the same composed source/digest list
  ...
  trust model.
- Python provider-wrapper attribution now follows exact one-hop selected local package reexports for
  AgentScope/PydanticAI symbols when the reexport alias is not rebound; wildcard, transitive, and
  rebound forms remain unresolved.
