...
- OpenAI Agents JS tool guardrails can safely inherit reject-condition literals through exact helper
  chains. A unique helper whose single return expression is a direct call to another proven helper
  can reuse the callee's literal/source metadata while preserving the wrapper helper name; cycles,
  multiple returns, compound expressions, and unresolved imports remain outside the summary.
...
- OpenAI Agents JS hosted web-search scope is now reviewable when it is literal. Exact
  ...
  the `external-action` capability; stable same-file, imported sibling, named-reexported, and
  unambiguous star-reexported const options objects now expose the same policy metadata with
  `web_search_options_binding` provenance. Exact literal
  ...
  context sizes, mutable options objects, nested-mutated imported options, ambiguous star-reexported
  options, and dynamic provider-data include arrays remain unresolved; dynamic `userLocation`
  ...
  options extensions are covered in the current 2,621-label IR corpus.
...
- OpenAI Agents JS `hostedMcpTool({...})` approval settings are now visible as hosted-MCP-specific
  ...
  policies, stable same-file const object policies, exact imported local const object policies,
  exact named local reexported const object policies, and unambiguous local star-reexported const
  object policies record exact `never`/`always` tool-name branches and read-only hints, and
  ...
  `startsWith`/`includes` string-method predicates, direct array-literal membership checks, and
  exact negated array-literal membership checks, and exact negated `startsWith`/`includes`
  method calls now record shallow predicate metadata, while local-variable/local-array lookalikes
  and opaque callback helpers are inventoried without request-review proof. The local fixture plus
  pinned OpenAI hosted MCP examples cover 36/36 hosted-MCP approval labels in the current
  2,621-label IR corpus.
