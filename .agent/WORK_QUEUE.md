...
- Explore richer OpenAI Agents JS tracing only if it can stay source-proven: trace processors,
  durable trace/export sinks, exported Runner factories, delegated `runConfig.groupId` or trace
  metadata policy, or governance edges that can connect `trace-group`/`trace-id`/`tracing-disabled`
  evidence to actual audit storage without treating observability IDs as memory/session continuity.
