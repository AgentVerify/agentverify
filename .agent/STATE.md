...
- Extended TypeScript OpenAI Agents JS hosted-MCP approval policy/callback coverage through
  ...
  preserves exact `never`/`always` tool-name lists, read-only hints, binding provenance, and
  ...
  literal-set predicates, destructured callback-parameter equality, and nested numeric/boolean
  request-field literal predicates record shallow predicate metadata, and local-variable/local-array
  lookalikes plus opaque call-result callbacks avoid overclaiming request review proof. Focused
  `IR-TS-OPENAI-HOSTED-MCP-APPROVAL` labels pass 31/31, the full public IR truth set passes
  2,616/2,616, and benchmark verification passes 3,349/3,349
  combined public-regression labels. The full pytest suite passes 368/368, and a post-refresh
  warm-cache IR pass hits 157/157 targets.
- Extended TypeScript OpenAI Agents JS `webSearchTool(...)` policy extraction through stable
  ...
  while mutable same-file, nested-mutated imported, and ambiguous star-reexported option bindings
  remain unresolved. Focused `IR-TS-OPENAI-WEB-SEARCH-POLICY` labels pass 34/34, the full public IR
  truth set passes 2,601/2,601, and benchmark verification passes 3,334/3,334 combined
...
- Extended Vercel `WorkflowAgent` execute-helper capability propagation through exact
  ...
  full pytest suite passes 368/368 at that milestone.
...
- Extended TypeScript OpenAI Agents JS tool-guardrail helper predicates through stable helper-result
  bindings. Reject-content branches now inherit exact helper metadata when an `if` condition uses a
  local boolean binding assigned from a proven helper call, provided the binding is not reassigned
  before the branch. Focused `IR-TS-OPENAI-TOOL-GUARDRAILS` labels pass 53/53, the full reporting
  truth set passed 733/733 before the helper-chain expansion; the current aggregate
  public-regression counts are recorded in the latest entry above.
- Extended TypeScript OpenAI Agents JS tool-guardrail helper predicate metadata through exact
  ...
  Focused `IR-TS-OPENAI-TOOL-GUARDRAILS` labels passed 50/50 before the helper-result binding
  expansion; the current aggregate public-regression counts are recorded in the latest entry above.
