...
SPEC = importlib.util.spec_from_file_location(
    "export_editor_contracts", ROOT / "scripts/export_editor_contracts.py"
)
assert SPEC is not None
export_editor_contracts = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(export_editor_contracts)


def test_editor_contract_export_writes_valid_rules_and_sample_report(tmp_path: Path) -> None:
    manifest = export_editor_contracts.export_editor_contracts(
        tmp_path,
