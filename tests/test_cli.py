...
def test_cli_benchmark_verify_summary_format(capsys) -> None:
    ...
    assert captured.out == (
        ...
        "Results: 2\n"
        "Labels: 3305/3305 passed (0 failed)\n"
        "Digests: 2/2 ok\n"
        "- benchmarks/truthset-results.json: 733/733 passed, reporting-rules, public-regression\n"
        "- benchmarks/ir-truthset-results.json: 2572/2572 passed, agent-ir, public-regression\n"
    )
...
def test_cli_benchmark_verify_writes_output_file(tmp_path: Path, capsys) -> None:
    ...
    assert captured.err == ""
    payload = __import__("json").loads(output.read_text(encoding="utf-8"))
    assert payload["results"][0]["labels"] == 733
...
def test_cli_benchmark_verify_engine_rejects_metric_summary_drift(
    tmp_path: Path, capsys
) -> None:
    payload = json.loads((ROOT / "benchmarks/engine-results.json").read_text(encoding="utf-8"))
    payload["summary"]["typescript_openai_codex_tool"]["components"] += 1
    result = tmp_path / "engine-results.json"
    result.write_text(json.dumps(payload), encoding="utf-8")
    ...
    assert captured.out == ""
    assert (
        "summary.typescript_openai_codex_tool.components does not match ok repository total"
        in captured.err
    )
