...
- `cases/typescript_flowise_secure_request`: a variable Flowise node URL reaches a fixed Axios
  request object and default-on pinned helper; default, redirect, lookup, deny-predicate,
  caller-transport, mapped-IP, and unrelated same-named-helper negatives withhold the policy edge.
...
snapshot. It parsed 10,747 selected Python/TypeScript/JavaScript files plus 155 configuration files,
resolved 1,469 relationships, and completed in 123.37 seconds on the development machine. Three parse
...
URL-security call sites, all charged against the same cap. This refresh materialized 152 dependency files across 14
...
dependency count per repository; engine schema v36 carries both the 152-file total and the
14-repository coverage.

Engine benchmark schema v36 retains stable component-name taxonomies, category presence counts,
...
The benchmark now also measures identity coverage: 8,738 agent/tool component observations carry
module-qualified IDs. Of 2,938 relationship endpoints, 1,930 carry symbol IDs and 1,928 resolve to an
observed component (1,675 Python and 253 TypeScript). Schema v36 records 325 same-scope and 14
...
Schema-v36 benchmark output measures native AI BOM endpoint resolution separately. AI BOM 1.1
resolves 1,928 endpoints by symbol ID, 463 by exact evidence location, and 18 by a unique display
name; 30 remain ambiguous and 499 unresolved. Before evidence-local and occurrence-qualified
...
tools plus MCP `registerTool` names and callback spans. Schema v36 records 27 Mastra factory tools,
...
helper summaries: three Mastra static methods and one MCP Servers free function. Schema v36 also
...
15 repositories. Schema v36 proves seven receivers: two exact Playwright `Page` annotations in
...
unresolved. Schema v36 publishes the resulting fixed-binding split as five instance edges and five
...
manager attribute, a fallback manager, and a rebound imported constructor. Schema v36 distinguishes
...
writes and fixed/configured tool paths do not trigger it. The full benchmark reports 26 default-scope sites across nine
...
Schema v36 therefore records five Python and one TypeScript boundary edges, all with unresolved root
...
Schema v36 additionally resolves 20 `Path.rename`/`Path.replace` moves: two renames and 18 replaces.
...
parameter (or its shallow assignment/destructuring alias) determines the URL origin. Two exact
TypeScript composition families additionally propagate tool URLs through imported Axios helpers. A
literal URL and a template/concatenation whose resolved literal prefix already contains a complete
HTTP scheme and host remain inventory-only. The full benchmark reports 15 reviews across nine
repositories:
...
- Flowise's Agentflow HTTP node copies its variable URL into a fixed Axios request object and calls
  the imported `secureAxiosRequest`; the proxy-conditional policy is modeled separately below.

Schema v36 separately inventories 22 import-proven `urllib.request.urlopen` calls. Two occur in
...
Schema v36 also resolves CrewAI's locally defined `safe_get` as a `network-ssrf-policy` at two
...
Schema v36 adds two production Composio edges from the session-file router to `safe_get` and
...
Schema v36 adds ten Langflow production edges whose policy is strong when enabled but explicitly
...
Schema v36 resolves n8n's AI Builder `web_fetch` as a dynamic-origin tool path and preserves its
...
Schema v36 also resolves Flowise's `httpAgentflow` node through its request object into
`secureAxiosRequest`. One audited caller hint is charged against Flowise's dependency budget. The
node exposes `nodeData.inputs.url` as a variable input, derives `finalUrl`, and assigns it to the
...
`escape_hatch: configured-opt-out`. Two positive and one negative IR labels cover the local and
pinned edges plus an unrelated same-named helper; mutations of the default, redirect mode, deny
predicate, pinned lookup, caller transport config, or mapped-address normalization withhold the edge.
...
`benchmarks/truthset.json` contains 305 exact labels across all nine enabled rules: 168 positives and 137
...
Compose/Kubernetes/Docker SDK settings. All 305 currently pass; each rule's seed precision and recall
...
edges plus raw Axios. Three Flowise request-object labels cover the local and pinned governed paths
plus an unrelated same-named helper. All 218 IR
labels pass:
