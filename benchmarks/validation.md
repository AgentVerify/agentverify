...
- `cases/typescript_openai_agent_guardrails`: OpenAI Agents JS Agent guardrails cover inline arrays,
...
- `cases/typescript_openai_realtime_session_guardrails`: OpenAI RealtimeSession output guardrails
  cover direct options, typed session-options spreads, inline arrays, same-file typed
  `RealtimeOutputGuardrail[]` arrays, exact relative imported typed arrays, exact named reexports,
  debounce settings, direct tripwire metadata, mutable-array negatives, and ambiguous barrel
  negatives.
...
All 733 cross-rule labels pass (337 positives and 396 negatives).
...
`benchmarks/truthset.json` contains 733 exact labels across all 25 enabled rules: 337 positives and 396
...
package launchers. All 733 currently pass;
...
Thirty-six OpenAI Agents JS hosted-MCP approval labels now pin disabled defaults, explicit
...
predicate metadata, request-field prefix/contains/literal-set and negated literal-set predicate
metadata, negated prefix/contains method predicate metadata, local-variable/local-array predicate
over-attribution negatives, opaque callback negatives without review proof, ambiguous-star and
mutated-binding negatives, and real hosted-MCP examples with callback approval metadata.
All 2,621 IR labels pass (2,016 positives and 605 negatives). The checked
