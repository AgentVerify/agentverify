...
truth set to 1,789 passing labels without broadening provider identity inference or conflating
...
agent session edges, Runner option-level session config, delegated `asTool` runConfig sessions, and
extension Runner backends, with host-local execution risk.
