...
resolved 1,385 relationships, and completed in 78.63 seconds on the development machine. Three parse
...
repository; engine schema v16 carries both the 135-file total and the 11-repository coverage.

Engine benchmark schema v16 retains stable component-name taxonomies, category presence counts,
...
module-qualified IDs. Of 2,770 relationship endpoints, 1,881 carry symbol IDs and 1,879 resolve to an
observed component (1,628 Python and 251 TypeScript). Schema v16 records 325 same-scope and 14
...
Schema-v16 benchmark output measures native AI BOM endpoint resolution separately. AI BOM 1.1
resolves 1,879 endpoints by symbol ID, 344 by exact evidence location, and 17 by a unique display
...
tools plus MCP `registerTool` names and callback spans. Schema v16 records 27 Mastra factory tools,
...
helper summaries: three Mastra static methods and one MCP Servers free function. Schema v16 also
...
unresolved. Schema v16 publishes the resulting fixed-binding split as five instance edges and five
...
manager attribute, a fallback manager, and a rebound imported constructor. Schema v16 distinguishes
...
fixed tool paths do not trigger it. The full benchmark reports 24 default-scope sites across eight
...
candidate cannot escape through `.parent`. Schema v16 therefore records one Python and one TypeScript boundary edge, both unresolved
in the pinned corpus.

Canonical and top-level import-aliased `os`/`shutil` mutations extend the Python sink model beyond
`open()` and `Path` methods. The full inventory contains 427 such calls: 179 creates, 190 deletes, 45
copies, and 13 moves. Two-path APIs use the destination argument, including keyword `dst`, and calls
record their canonical API, operation, and path role. This exposes five additional ArcadeAI reviews
at `os.replace`, `shutil.move`, and `shutil.copytree`, plus CrewAI Examples' `copytree`, raising the
rule total by six without adding a repository. A fixed destination and a boundary-guarded copy are
local negatives; import aliases are positive fixtures, while a shadowed `shutil` parameter and
string `.replace()` are explicit near-miss negatives.
...
`benchmarks/truthset.json` contains 196 exact labels across all eight enabled rules: 115 positives and 81
...
Compose/Kubernetes/Docker SDK settings. All 196 currently pass; each rule's seed precision and recall
...
control edge. Eight Python path-boundary labels cover three local proven sinks, three local bypasses,
a configured-root edge, and FastMCP's real control edge. Six Python filesystem-mutation labels cover
three local alias forms, a shadowed-module negative, and pinned ArcadeAI/CrewAI edges. Four closure-binding labels cover a returned local callback, a rebound negative, browser-use's
...
All 70 IR labels pass: three approval positives/four negatives,
...
negative, seven path-boundary positives/five negatives, five filesystem-mutation positives/one
negative, two MCP-registry positives/one negative, three
