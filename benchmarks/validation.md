...
resolved 1,386 relationships, and completed in 82.57 seconds on the development machine. Three parse
...
repository; engine schema v17 carries both the 135-file total and the 11-repository coverage.

Engine benchmark schema v17 retains stable component-name taxonomies, category presence counts,
...
module-qualified IDs. Of 2,772 relationship endpoints, 1,882 carry symbol IDs and 1,880 resolve to an
observed component (1,629 Python and 251 TypeScript). Schema v17 records 325 same-scope and 14
...
Schema-v17 benchmark output measures native AI BOM endpoint resolution separately. AI BOM 1.1
resolves 1,880 endpoints by symbol ID, 345 by exact evidence location, and 17 by a unique display
...
tools plus MCP `registerTool` names and callback spans. Schema v17 records 27 Mastra factory tools,
...
helper summaries: three Mastra static methods and one MCP Servers free function. Schema v17 also
...
unresolved. Schema v17 publishes the resulting fixed-binding split as five instance edges and five
...
manager attribute, a fallback manager, and a rebound imported constructor. Schema v17 distinguishes
...
fixed tool paths do not trigger it. The full benchmark reports 25 default-scope sites across eight
...
candidate cannot escape through `.parent`. Schema v17 therefore records one Python and one TypeScript boundary edge, both unresolved
in the pinned corpus.

Canonical, top-level import-aliased, and statement-ordered local callable-aliased `os`/`shutil`
mutations extend the Python sink model beyond `open()` and `Path` methods. The full inventory contains
428 such calls: 179 creates, 190 deletes, 46 copies, and 13 moves. Two-path APIs use the destination
...
`benchmarks/truthset.json` contains 203 exact labels across all eight enabled rules: 118 positives and 85
...
Compose/Kubernetes/Docker SDK settings. All 203 currently pass; each rule's seed precision and recall
...
control edge. Nine Python path-boundary labels cover four local proven sinks, three local bypasses,
a configured-root edge, and FastMCP's real control edge. Thirteen Python filesystem-mutation labels
cover six local positives, four local rebinding/compatibility negatives, and three pinned
...
All 78 IR labels pass: three approval positives/four negatives,
...
negative, eight path-boundary positives/five negatives, nine filesystem-mutation positives/four
