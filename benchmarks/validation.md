...
resolved 1,442 relationships, and completed in 105.78 seconds on the development machine. Three parse
...
repository; engine schema v31 carries both the 135-file total and the 11-repository coverage.

Engine benchmark schema v31 retains stable component-name taxonomies, category presence counts,
...
with redirect, DNS, configured scope, and open defaults kept separate. It intentionally
...
module-qualified IDs. Of 2,884 relationship endpoints, 1,929 carry symbol IDs and 1,927 resolve to an
observed component (1,676 Python and 251 TypeScript). Schema v31 records 325 same-scope and 14
...
Schema-v31 benchmark output measures native AI BOM endpoint resolution separately. AI BOM 1.1
resolves 1,927 endpoints by symbol ID, 410 by exact evidence location, and 17 by a unique display
...
tools plus MCP `registerTool` names and callback spans. Schema v31 records 27 Mastra factory tools,
...
helper summaries: three Mastra static methods and one MCP Servers free function. Schema v31 also
...
15 repositories. Schema v31 proves seven receivers: two exact Playwright `Page` annotations in
...
unresolved. Schema v31 publishes the resulting fixed-binding split as five instance edges and five
...
manager attribute, a fallback manager, and a rebound imported constructor. Schema v31 distinguishes
...
Schema v31 therefore records five Python and one TypeScript boundary edges, all with unresolved root
...
Schema v31 additionally resolves 20 `Path.rename`/`Path.replace` moves: two renames and 18 replaces.
...
Schema v31 separately inventories 22 import-proven `urllib.request.urlopen` calls. Two occur in
...
a container annotation negative, and Skyvern's proven factory result. All 196 IR
labels pass:
...
positives/five negatives, and two imported-registry positives/three negatives.
