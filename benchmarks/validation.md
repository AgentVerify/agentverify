...
resolved 1,527 relationships, and completed in 183.99 seconds on the development machine. Three parse
...
dependency count per repository; engine schema v54 carries both the 168-file total and the
18-repository coverage.

Engine benchmark schema v54 retains stable component-name taxonomies, category presence counts,
...
Schema v54 retains A2A endpoint provenance as a separate authority class: four exact client-construction
...
Schema v54 also publishes immutable same-file Axios-instance metrics and the sixth TypeScript
...
module-qualified IDs. Of 3,054 relationship endpoints, all 2,175 identified symbol endpoints resolve
to an observed component (1,916 Python and 259 TypeScript). Schema v54 records 311
`lexical-single-definition` targets, 20 exact same-block dominating definitions, and three exact
same-class helper-return edges to two Agent source definitions. One import-proven typed tool
...
Schema-v54 benchmark output measures native AI BOM endpoint resolution separately. AI BOM 1.2
resolves 2,175 endpoints by symbol ID, 521 by exact evidence location, and 15 by a unique display
name; 29 remain ambiguous and 314 unresolved. Before evidence-local and occurrence-qualified
...
Schema v54 separately inventories OpenAI Agents Python's MCP approval default without widening the
...
Schema v54 reports one medium-severity, high-confidence production review in Skyvern Task v3. The
...
`append_rows` sink. Schema v54 records one available durable control, one audit-storage capability,
...
and commits an `ActionModel` to the SQLAlchemy `actions` table. Schema v54 records one production
...
Schema v54 raises one medium-severity, high-confidence `AV-AUDIT001` review at this exact production
...
edges. Eighteen typed-tool-parameter labels cover exact assigned and inline-only local plus pinned
parameter edges, the non-collapsing concrete-ID negative, and mismatched, reassigned, uncalled,
rebound, class/nested-scope shadowed, union-typed, and wrong-annotation cases. All 340 IR labels pass:
...
Agent-helper-return positives/one negative, 16 typed-tool-parameter positives/two negatives, seven
