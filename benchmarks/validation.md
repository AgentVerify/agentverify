...
resolved 1,390 relationships, and completed in 90.07 seconds on the development machine. Three parse
...
repository; engine schema v20 carries both the 135-file total and the 11-repository coverage.

Engine benchmark schema v20 retains stable component-name taxonomies, category presence counts,
...
module-qualified IDs. Of 2,780 relationship endpoints, 1,882 carry symbol IDs and 1,880 resolve to an
observed component (1,629 Python and 251 TypeScript). Schema v20 records 325 same-scope and 14
...
Schema-v20 benchmark output measures native AI BOM endpoint resolution separately. AI BOM 1.1
resolves 1,880 endpoints by symbol ID, 353 by exact evidence location, and 17 by a unique display
...
tools plus MCP `registerTool` names and callback spans. Schema v20 records 27 Mastra factory tools,
...
helper summaries: three Mastra static methods and one MCP Servers free function. Schema v20 also
...
unresolved. Schema v20 publishes the resulting fixed-binding split as five instance edges and five
...
manager attribute, a fallback manager, and a rebound imported constructor. Schema v20 distinguishes
...
Schema v20 therefore records five Python and one TypeScript boundary edges, all with unresolved root
...
Schema v20 additionally resolves 20 `Path.rename`/`Path.replace` moves: two renames and 18 replaces.
...
`benchmarks/truthset.json` contains 228 exact labels across all eight enabled rules: 132 positives and 96
...
Compose/Kubernetes/Docker SDK settings. All 228 currently pass; each rule's seed precision and recall
...
and OpenAI's three real sinks. All 102 IR labels pass: three approval positives/four negatives,
