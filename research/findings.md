...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v32 resolves 448 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v32 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v32 inventories 80
...
Schema v32 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
and terminates the `ValueError` handler before writing. Schema v32 records this as a distinct
...
corpus. The current schema resolves 434 endpoints by exact relationship evidence. Reused Python
...
Schema v32 resolves only single direct definitions that appear earlier in the same Python lexical or
...
Schema v32 also inventories 22 exact `urllib.request.urlopen` calls. Two are reachable from Qwen's
...
Schema v32 separately models fail-closed Python scheme and hostname checks as
...
Schema v32 additionally proves CrewAI's `safe_get` transport at two production loader calls. Unlike
