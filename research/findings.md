...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v33 resolves 448 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v33 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v33 inventories 80
...
Schema v33 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
and terminates the `ValueError` handler before writing. Schema v33 records this as a distinct
...
corpus. The current schema resolves 438 endpoints by exact relationship evidence. Reused Python
...
Schema v33 resolves only single direct definitions that appear earlier in the same Python lexical or
...
Schema v33 also inventories 22 exact `urllib.request.urlopen` calls. Two are reachable from Qwen's
...
Schema v33 separately models fail-closed Python scheme and hostname checks as
...
Schema v33 additionally proves CrewAI's `safe_get` transport at two production loader calls. Unlike
