...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v47 resolves 449 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v47 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v47 inventories 80
...
Schema v47 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
and terminates the `ValueError` handler before writing. Schema v47 records this as a distinct
...
the BigQuery Storage Write API. Schema v47 therefore records one available durable audit control and
...
workflow, task, step, type, status, and ordering fields. Schema v47 records one production
...
failures are contained. Schema v47 raises one `AV-AUDIT001` review only on that explicit production
...
Schema v47 resolves only single direct definitions that appear earlier in the same Python lexical or
...
origin, so schema v47 emits two `AV-A2A001` reviews.
