...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v57 resolves 449 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v57 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v57 inventories 80
...
Schema v57 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
rule observations are pinned in the 349-label truth set.
...
and terminates the `ValueError` handler before writing. Schema v57 records this as a distinct
...
the BigQuery Storage Write API. Schema v57 therefore records one available durable audit control and
...
workflow, task, step, type, status, and ordering fields. Schema v57 records one production
...
failures are contained. Schema v57 raises one `AV-AUDIT001` review only on that explicit production
...
Schema v57 resolves 357 repeated-name targets from one earlier lexical definition, 20 targets from a
...
recover 707 role-proven tool components: 177 callables, 91 constructor-bound instances, 437 inline
constructors, and two direct context-manager bindings. They create 762 exact Agent edges; 507 tools
are outside tests and 21 capability edges become reachable. Import-proven OpenAI `function_tool(function)` assignments
...
and a direct Agent return. The final export resolves 3,265 endpoints by symbol ID, 535 by exact
evidence location, and 26 by unique display name; 21 remain ambiguous control/tool targets, and 109
...
module and function imports now withhold those IDs, so all 3,265 identified endpoints resolve.
...
origin, so schema v57 emits two `AV-A2A001` reviews.
