...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v48 resolves 449 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v48 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v48 inventories 80
...
Schema v48 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
and terminates the `ValueError` handler before writing. Schema v48 records this as a distinct
...
the BigQuery Storage Write API. Schema v48 therefore records one available durable audit control and
...
workflow, task, step, type, status, and ordering fields. Schema v48 records one production
...
failures are contained. Schema v48 raises one `AV-AUDIT001` review only on that explicit production
...
Schema v48 resolves only single direct definitions that appear earlier in the same Python lexical or
module scope: 325 same-scope and 14 module-scope edges. The final export resolves 1,944 endpoints by
symbol ID, 496 by exact evidence location, and 23 by unique display name; 38 remain ambiguous
control/tool targets, and 501 unresolved. Two apparent CrewAI re-export misses were false identities:
a function parameter and a local assignment shadowed the imported `tool` binding. Scope-isolating
module and function imports now withholds those IDs, so all 1,944 identified endpoints resolve.
...
origin, so schema v48 emits two `AV-A2A001` reviews.
