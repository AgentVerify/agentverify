...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v45 resolves 449 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v45 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v45 inventories 80
...
Schema v45 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
and terminates the `ValueError` handler before writing. Schema v45 records this as a distinct
...
the BigQuery Storage Write API. Schema v45 therefore records one available durable audit control and
...
unresolved. This positive control evidence does not justify an absence-based `AV-AUDIT001` finding.
...
corpus. The current schema resolves 492 endpoints by exact relationship evidence. Reused Python
...
Schema v45 resolves only single direct definitions that appear earlier in the same Python lexical or
module scope: 325 same-scope and 14 module-scope edges. The final export resolves 1,939 endpoints by
symbol ID, 492 by exact evidence location, and 20 by unique display name; 38 remain ambiguous
...
origin, so schema v45 emits two `AV-A2A001` reviews.
