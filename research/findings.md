...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v41 resolves 449 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v41 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v41 inventories 80
...
Schema v41 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
The collector's bounded dependency closure and audited evidence hints add 159 local source files
across 17 repositories. It
...
rule observations are pinned in the 330-label truth set.
...
and terminates the `ValueError` handler before writing. Schema v41 records this as a distinct
...
corpus. The current schema resolves 481 endpoints by exact relationship evidence. Reused Python
...
Schema v41 resolves only single direct definitions that appear earlier in the same Python lexical or
module scope: 325 same-scope and 14 module-scope edges. The final export resolves 1,934 endpoints by
symbol ID, 481 by exact evidence location, and 19 by unique display name; 37 remain ambiguous
...
Seventeen default-scope tool paths in the bounded corpus pass a tool parameter directly or through a
...
origin, so schema v41 emits two `AV-A2A001` reviews.
