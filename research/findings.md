...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v30 resolves 448 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v30 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v30 inventories 80
...
Schema v30 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
rule observations are pinned in the 298-label truth set.
...
and terminates the `ValueError` handler before writing. Schema v30 records this as a distinct
...
Schema v30 resolves only single direct definitions that appear earlier in the same Python lexical or
...
Schema v30 also inventories 22 exact `urllib.request.urlopen` calls. Two are reachable from Qwen's
...
Schema v30 separately models fail-closed Python scheme and hostname checks as
