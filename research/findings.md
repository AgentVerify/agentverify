...
proven transparent wrappers—yield only 22 capability edges and
...
Browser-page evaluation is a separate execution boundary. The corpus contains 93 import-context
calls across supported Playwright evaluator APIs. The original 86 `.evaluate(...)` calls retain 41
proven receivers under schema v90: two exact Playwright `Page` parameters in
...
Browser-Use contributes the new proof: one uniquely annotated module page is imported under a
fail-closed dependency guard and flows to a Locator assignment that dominates its evaluation within
the same `else` branch.
Schema v91 structurally adds `evaluate_handle(...)`, `eval_on_selector(...)`,
`eval_on_selector_all(...)`, and Locator `evaluate_all(...)`, selecting the script by API-specific
position or exact `expression=` keyword. Seven Skyvern test observations add three locally
constructed-page proofs and four unresolved receivers, bringing the totals to 44 proven and 49
unresolved; the API split is 86 `evaluate`, five `eval_on_selector`, and two
`eval_on_selector_all`. A dynamic selector with a fixed expression is inventory-only. The selected
production alternate-API call lacks local Playwright provenance and remains outside the inventory,
so the dynamic production path and `AV-EXEC002` finding count are unchanged.
Schema v92 proves four more fixed-script Skyvern test receivers through two unique local
...
remain unchanged.
Schema v98 proves Skyvern's fixed-script popup probe at
[`scouting.py:1508`](https://github.com/Skyvern-AI/skyvern/blob/486c8975e9864a53037d4701b781f8619e698c40/skyvern/forge/sdk/copilot/tools/scouting.py#L1508).
The imported `AgentContext` uniquely annotates `pending_scout_popup` as `Page | None`; the consumer
uses an exact immutable `AgentContext` parameter and copies that field into one immutable local
...
Skyvern's direct and Locator scroll evaluators at lines 1664 and 1662 are real negatives because the
generated JavaScript contains only normalized numeric intermediates. Literal scripts, an ordinary `.evaluate` method even inside
...
created. Exact `WebSearchTool`, `FileSearchTool`, and `ImageGenerationTool` imports add 13
...
module and function imports now withhold those IDs, so all 3,946 identified endpoints resolve.
