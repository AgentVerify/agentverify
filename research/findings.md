...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v46 resolves 449 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v46 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v46 inventories 80
...
Schema v46 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
rule observations are pinned in the 336-label truth set.
...
and terminates the `ValueError` handler before writing. Schema v46 records this as a distinct
...
the BigQuery Storage Write API. Schema v46 therefore records one available durable audit control and
...
workflow, task, step, type, status, and ordering fields. Schema v46 records one production
...
failures are contained. These positive controls do not justify an absence-based `AV-AUDIT001` finding.
...
Schema v46 resolves only single direct definitions that appear earlier in the same Python lexical or
...
control/tool targets, and 501 unresolved.
...
origin, so schema v46 emits two `AV-A2A001` reviews.
