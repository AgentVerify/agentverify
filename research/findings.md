...
`.evaluate(...)` calls. Schema v90 proves 41 receivers: two exact Playwright `Page` parameters in
...
are withheld. The remaining 45 fixed-script observations retain explicit unresolved receiver state. Only one proven
...
rule observations are pinned in the consolidated 557-label truth set.
