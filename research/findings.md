...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v49 resolves 449 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v49 preserves
...
Browser-page evaluation is a separate execution boundary. Schema v49 inventories 80
...
Schema v49 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
and terminates the `ValueError` handler before writing. Schema v49 records this as a distinct
...
the BigQuery Storage Write API. Schema v49 therefore records one available durable audit control and
...
workflow, task, step, type, status, and ordering fields. Schema v49 records one production
...
failures are contained. Schema v49 raises one `AV-AUDIT001` review only on that explicit production
...
corpus. The current schema resolves 496 endpoints by exact relationship evidence. Reused Python
...
Schema v49 resolves only single direct definitions that appear earlier in the same Python lexical or
module scope (325 same-scope and 14 module-scope edges), or one constructor assignment that is the
sole same-block mutation before use (13 CrewAI edges). The final export resolves 1,957 endpoints by
symbol ID, 496 by exact evidence location, and 23 by unique display name; 38 remain ambiguous
control/tool targets, and 488 unresolved. Ten parameter, helper-return, factory, tuple-unpack,
cross-branch, or reassigned repeated-binding targets remain withheld. Two apparent CrewAI re-export
misses were false identities:
a function parameter and a local assignment shadowed the imported `tool` binding. Scope-isolating
module and function imports now withhold those IDs, so all 1,957 identified endpoints resolve.
...
origin, so schema v49 emits two `AV-A2A001` reviews.
