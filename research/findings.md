...
Filesystem mutation is broader than `open()` and `write_text()`. Schema v22 resolves 448 Python
...
That copy and three sibling sinks rely on `str(resolved).startswith(str(root))`; schema v22 preserves
...
descendant boundary. Across Skyvern, 93 recovered registrations yield only 15 capability edges and
...
Schema v22 therefore reports five instance and five closure fixed-binding edges, without suppressing
any review.
...
rule observations are pinned in the 236-label truth set.
...
and terminates the `ValueError` handler before writing. Schema v22 records this as a distinct
